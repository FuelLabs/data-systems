...
impl TimeRange {
    pub fn time_since_now(&self) -> DateTime<Utc> {
        ...
        let start_time = match self {
            Self::OneHour => now - chrono::Duration::hours(1),
            Self::TwelveHours => now - chrono::Duration::hours(12),
            Self::OneDay => now - chrono::Duration::days(1),
            Self::SevenDays => now - chrono::Duration::days(7),
            Self::ThirtyDays => now - chrono::Duration::days(30),
            Self::NinetyDays => now - chrono::Duration::days(90),
            Self::OneEightyDays => now - chrono::Duration::days(180),
            Self::OneYear => now - chrono::Duration::days(365),
            Self::All => now,
        };
        start_time
    }
...
impl BlocksQuery {
    pub fn get_sql_and_values(&self) -> (String, sea_query::Values) {
        self.build_query().build(PostgresQueryBuilder::default())
    }
