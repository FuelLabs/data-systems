...
impl SubjectValidator {
    ...
    pub fn to_sql_pattern(pattern: &str) -> String {
        if pattern.contains('>') {
            // Greater than matches everything after
            let res = pattern.replace('>', "%");
            return res;
        }

        // For * we need to ensure it matches exactly one segment
        let segments: Vec<&str> = pattern.split('.').collect();
        let converted: Vec<String> = segments
            .iter()
            .map(|&seg| {
                if seg == "*" {
                    "[^.]+".to_string() // Match one or more non-dot characters
                } else {
                    regex::escape(seg) // Escape any regex special characters
                }
            })
            .collect();

        // Add ^ and $ to ensure we match the entire string
        let res = format!("^{}$", converted.join("\\."));
        res
    }
