...
use crate::{
    fuel_streams::{FuelStreams, FuelStreamsExt},
    publisher::{publish, PublishOpts},
};

pub async fn publish_all_tasks(
    ...
    for tx_item @ (_, tx) in transactions.iter().enumerate() {
        ...
        let client_tx = client.transaction(&tx_id).await?;
        if client_tx.is_none() {
            continue;
        }

        let client_tx = client_tx.unwrap();
        let client_tx_status = client_tx.status.clone();
        let receipts = match client_tx_status {
            ClientTransactionStatus::Success { receipts, .. } => receipts,
            ClientTransactionStatus::Failure { receipts, .. } => receipts,
            _ => vec![],
        };
        ...
        tasks.extend(publish_tasks(
            tx_item,
            &tx_id.into(),
            &client_tx.status.into(),
            fuel_streams.transactions(),
            opts,
            &receipts,
        ));
        tasks.extend(publish_inputs(
            tx,
            &tx_id.into(),
            fuel_streams.inputs(),
            opts,
        ));
        tasks.extend(publish_outputs(
            tx,
            &tx_id.into(),
            fuel_streams.outputs(),
            opts,
        ));
