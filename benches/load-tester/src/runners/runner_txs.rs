...
pub async fn run_txs_consumer(
    client: &Client,
    load_test_tracker: Arc<LoadTestTracker>,
) -> Result<()> {
    // Create a new stream for txs
    let stream = fuel_streams::Stream::<Transaction>::new(client).await;

    // Configure the stream to start from the last published tx
    let config = StreamConfig {
        deliver_policy: DeliverPolicy::Last,
    };

    // Subscribe to the txs stream with the specified configuration
